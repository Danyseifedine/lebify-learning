...
{
    protected $signature = 'update:view-role-permission';
    protected $description = 'Update view for role and permission';

    public function handle()
    {
        $this->updatePermissionView();
        $this->info('View files have been updated successfully.');
    }

    private function updatePermissionView()
    {
        $permissionViewPath = resource_path('views/dashboard/pages/privileges/permission.blade.php');
        $roleViewPath = resource_path('views/dashboard/pages/privileges/role.blade.php');

        if (!File::exists($permissionViewPath)) {
            $this->error('permission.blade.php view file does not exist.');
            return;
        }
        if (!File::exists($roleViewPath)) {
            $this->error('role.blade.php view file does not exist.');
            return;
        }

        $permissionViewContent = $this->getPermissionViewContent();
        File::put($permissionViewPath, $permissionViewContent);

        $roleViewContent = $this->getRoleViewContent();
        File::put($roleViewPath, $roleViewContent);

        $this->info('permission.blade.php view file has been updated.');
        $this->info('role.blade.php view file has been updated.');
    }

    private function getPermissionViewContent()
    {
        return <<<'EOD'
...
EOD;
    ...
    private function getRoleViewContent()
    {
        return <<<'EOD'
@extends('layouts.dashboard')
...
@section('content')
    <div class="row g-6 g-xl-9">
        <div class="card">
            <div class="card-body">
                ...
                <!-- start table -->
                <table id="role-datatable" class="table align-middle table-row-dashed fs-6 gy-5">
                    <thead>
                        <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                            <th class="w-10px pe-2">
                                <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
    ...
    <div class="modal fade" tabindex="-1" id="create-modal" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn close-modal btn-light">Close</button>
                    <button type="button" with-spinner="true" class="btn btn-primary" id="create-role-button">
                        <span class="ld-span">Create</span>
                    </button>
                </div>
            </div>
    ...
    <div class="modal fade" tabindex="-1" id="edit-modal" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn close-modal btn-light">Close</button>
                    <button type="button" with-spinner="true" class="btn btn-primary" id="edit-role-button">
                        <span class="ld-span">Edit</span>
                    </button>
                </div>
            </div>
    ...
    <!-- start show modal -->
    <div class="modal fade" tabindex="-1" id="role-permission-modal" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div id="show-role-permission-content">
                        <div id="loading-spinner" class="text-center">
                            <div class="spinner-border" role="status">
                                <span class="visually-hidden">Loading...</span>
                            </div>
