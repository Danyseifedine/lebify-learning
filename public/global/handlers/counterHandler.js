...
export class CounterHandler extends Identifiable {
    ...
    initializeCounters() {
        ...
        counters.forEach(counterElement => {
            ...
            if (display) {
                ...
                errorLogger.logMissingAttributeError('SimpleCounter', this.config.attributes.display, 'counter', 'as a child element', 'Required to display the counter value');
            }
        });
    }
    ...
    getCounterConfig(element) {
        ...
        if (Object.values(config).some(value => typeof value === 'number' && isNaN(value))) {
            errorLogger.logError('SimpleCounter: Invalid configuration values');
            return null;
        }
        return config;
    }
