...
import { errorLogger } from '../utils/classes/error-utils.js';
...
export class TabHandler extends Identifiable {
    ...
    init() {
        ...
        if (!tabLists.length) {
            errorLogger.logError('No tab lists found');
            return;
        }
    ...
    initTabGroup(tabList) {
        ...
        if (!tabGroupId) {
            errorLogger.logMissingAttributeError(
                'TabHandler',
        ...
        if (this.tabGroups.has(tabGroupId)) {
            errorLogger.logError(`Duplicate tab group ID found: ${tabGroupId}. Each tab group must have a unique ID.`);
            return;
        }
    ...
    async loadTabContent(tabGroupId, tabId) {
        ...
        try {
            ...
            errorLogger.logError(`Error loading content for tab ID ${tabId} in group ${tabGroupId}:`, error);
