/*=============================================================================
 * CourseLesson Management Module
 *
 * This module handles all courseLesson-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for courseLesson operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => courselessonTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/courses/lessons/${path}`;
...
const userActionHandlers = {
    ...
    edit: function (id) {
        this.callCustomFunction('_EDIT_', id, buildApiUrl(`${id}/edit`), (response) => {
            initSelect2('#language', "#edit-modal");
            initSelect2('#course_id', "#edit-modal");
        });
    },
...
const uiEventListeners = [
    ...
    { event: 'click', selector: '.toggle-status', handler: userActionHandlers.status },
];

/*---------------------------------------------------------------------------
 * DataTable Configuration
 * Defines the structure and behavior of the CourseLesson management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    ...
    { targets: [5], orderable: false, htmlType: 'toggle', dataClassName: "toggle-status" },
    {
        targets: [-1],
        htmlType: 'actions',
        className: 'text-end',
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected courseLessons
 * @param {Array} selectedIds - Array of selected courseLesson IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple courseLessons, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main courseLesson management interface
 *============================================================================*/

// Add a namespace prefix for filter selects
const FILTER_PREFIX = 'filter_';

export const courselessonTable = new $DatatableController('courselessonTable', {
...
// Initialize create courseLesson modal
createModalLoader({
    triggerSelector: '.create',
    endpoint: buildApiUrl('create'),
    onSuccess: (response) => {
        initSelect2('#language', "#create-modal");
        initSelect2('#course_id', "#create-modal");
    }
});
