/*=============================================================================
 * QuizAttempt Management Module
 *
 * This module handles all quizAttempt-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for quizAttempt operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => quizAttemptTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/quiz/attempts/${path}`;
 ...
 * Defines the structure and behavior of the QuizAttempt management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    ...
    },
    {
        ...
        customRender: (data, type, row) => {
            if (row.passed) {
                return `<span class="badge bg-light-success">Passed</span>`;
            } else {
                return `<span class="badge bg-light-danger">Failed</span>`;
            }
        }
    },
    {
        ...
        customRender: (data, type, row) => {
            if (row.status === 'completed') {
                return `<span class="badge text-white bg-success">Completed</span>`;
            }
            if (row.status === 'pending') {
                return `<span class="badge text-white bg-warning">Pending</span>`;
            }
            if (row.status === 'timeout') {
                return `<span class="badge text-white bg-danger">Timeout</span>`;
            }
            if (row.status === 'aborted') {
                return `<span class="badge text-white bg-info">Aborted</span>`;
            }
        }
    },
    {
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected quizAttempts
 * @param {Array} selectedIds - Array of selected quizAttempt IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple quizAttempts, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main quizAttempt management interface
 *============================================================================*/
export const quizAttemptTable = new $DatatableController('quizAttemptTable', {
...
// Initialize create quizAttempt modal
createModalLoader({
