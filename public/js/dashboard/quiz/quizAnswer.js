/*=============================================================================
 * QuizAnswer Management Module
 *
 * This module handles all quizAnswer-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for quizAnswer operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => quizAnswersTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/quiz/questions/answers/${path}`;
...
const userActionHandlers = {
    ...
    edit: function (id) {
        this.callCustomFunction('_EDIT_', id, buildApiUrl(`${id}/edit`), (response) => {
            initSelect2("#question_id", "#edit-modal");
        });
    },
    ...
    isCorrect: function (id) {
        this.callCustomFunction('_PATCH_', buildApiUrl(`${id}/is-correct`), (response) => {
            // console.log(response);
        });
    }
};
...
const uiEventListeners = [
    ...
    { event: 'click', selector: '.btn-is-correct', handler: userActionHandlers.isCorrect },
];

/*---------------------------------------------------------------------------
 * DataTable Configuration
 * Defines the structure and behavior of the QuizAnswer management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    ...
    { targets: [3], orderable: false, htmlType: 'toggle', dataClassName: 'btn-is-correct' },
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected quizAnswers
 * @param {Array} selectedIds - Array of selected quizAnswer IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple quizAnswers, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main quizAnswer management interface
 *============================================================================*/

const FILTER_PREFIX = "filter_"
export const quizAnswersTable = new $DatatableController('quizAnswersTable', {
    ...
    ajax: {
        ...
        data: (d) => ({
            ...d,
            filter_question_id: $("#filter_question_id").val()
        })
    },
...
// Initialize create quizAnswer modal
createModalLoader({
    triggerSelector: '.create',
    endpoint: buildApiUrl('create'),
    onSuccess: () => {
        initSelect2("#question_id", "#create-modal");
    }
});
