/*=============================================================================
 * QuizQuestion Management Module
 *
 * This module handles all quizQuestion-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for quizQuestion operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => quizQuestionTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/quiz/questions/overview/${path}`;
...
const userActionHandlers = {
    ...
    edit: function (id) {
        this.callCustomFunction('_EDIT_', id, buildApiUrl(`${id}/edit`), (response) => {
            initSelect2("#quiz_id", "#edit-modal");
            initSelect2("#category_id", "#edit-modal");
            initSelect2("#type", "#edit-modal");
        });
    },
 ...
 * Defines the structure and behavior of the QuizQuestion management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    ...
    { targets: [4], orderable: false, htmlType: 'badge', badgeClass: 'badge-light-primary' },
    {
        targets: [-1],
        htmlType: 'actions',
        className: 'text-end',
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected quizQuestions
 * @param {Array} selectedIds - Array of selected quizQuestion IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple quizQuestions, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main quizQuestion management interface
 *============================================================================*/

const FILTER_PREFIX = "filter_";

export const quizQuestionTable = new $DatatableController('quizQuestionTable', {
    ...
    ajax: {
        ...
        data: (d) => ({
            ...d,
            filter_quiz_id: $('#filter_quiz_id').val(),
        })
    },
...
// Initialize create quizQuestion modal
createModalLoader({
    triggerSelector: '.create',
    endpoint: buildApiUrl('create'),
    onSuccess: (response) => {
        initSelect2("#quiz_id", "#create-modal");
        initSelect2("#category_id", "#create-modal");
        initSelect2("#type", "#create-modal");
    }
});
