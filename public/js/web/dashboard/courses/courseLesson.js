...
const courseLessonDataTable = new $DatatableController('courseLesson-datatable', {
    ...
    ajax: {
        url: `${__API_CFG__.LOCAL_URL}/dashboard/course/lessons/datatable`,
        data: (d) => ({
            ...d,
    ...
    customFunctions: {
        ...
        _SHOW_: async function (id, endpoint, onSuccess, onError) {
            console.log("Show courseLesson", id);
        },
...

    {
        ...
        handler: function (id, event) {
            this.callCustomFunction(
                '_DELETE_WITH_ALERT_',
                `${__API_CFG__.LOCAL_URL}/dashboard/course/lessons/delete/${id}`,
                (res) => {
                    if (res.risk) {
                        SweetAlert.error();
                    } else {
                ...
                (err) => {
                    console.error('Error deleting courseLesson', err);
                }
            );
        }
    },
...
function createCourseLesson() {
    FunctionUtility.closeModalWithButton('create-modal', '.close-modal', () => {
        FunctionUtility.clearForm('#create-courseLesson-form');
    });

    const createCourseLessonConfig = {
        formSelector: '#create-courseLesson-form',
        externalButtonSelector: '#create-courseLesson-button',
        endpoint: `${__API_CFG__.LOCAL_URL}/dashboard/course/lessons/store`,
        feedback: true,
        onSuccess: (res) => {
            ...
            courseLessonDataTable.reload();
        },
        onError: (err) => {
            console.error('Error adding courseLesson', err);
        },
    };

    const form = new $SingleFormPostController(createCourseLessonConfig);
    form.init();
}
createCourseLesson();
...
const editCourseLesson = () => {
    ...
    const editCourseLessonConfig = {
        formSelector: '#edit-courseLesson-form',
        externalButtonSelector: '#edit-courseLesson-button',
        endpoint: `${__API_CFG__.LOCAL_URL}/dashboard/course/lessons/edit`,
        feedback: true,
        onSuccess: (res) => {
            Toast.showNotificationToast('', res.message)
            FunctionUtility.closeModal('edit-modal', () => {
                FunctionUtility.clearForm('#edit-courseLesson-form');
            });
            courseLessonDataTable.reload();
        },
        onError: (err) => {
    ...
    const form = new $SingleFormPostController(editCourseLessonConfig);
    form.init();
}
editCourseLesson();
