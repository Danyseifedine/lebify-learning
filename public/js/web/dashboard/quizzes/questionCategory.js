...
const questionCategoryDataTable = new $DatatableController('questionCategory-datatable', {
    ...
    ajax: {
        url: `${__API_CFG__.LOCAL_URL}/dashboard/quizzes/question/category/datatable`,
        data: (d) => ({
            ...d,
    ...
    columnDefs: $DatatableController.generateColumnDefs([
        ...
        { targets: [1], htmlType: 'badge', badgeClass: 'badge-light-danger fs-6' },
        // example: {
    ...
    customFunctions: {
        ...
        _SHOW_: async function (id, endpoint, onSuccess, onError) {
            console.log("Show questionCategory", id);
        },
        ...
        _EDIT_WITH_MODAL_: async function (id, endpoint, onSuccess, onError) {
            const modalHandler = new $ModalFormFetchController({
                modalId: 'edit-modal',
                endpoint: `${endpoint}`,
                formId: '#edit-questionCategory-form',
    ...
    eventListeners: [
        {
            ...
            handler: function (id, event) {
                this.callCustomFunction(
                    '_DELETE_WITH_ALERT_',
                    `${__API_CFG__.LOCAL_URL}/dashboard/quizzes/question/category/delete/${id}`,
                    (res) => {
                        if (res.risk) {
                            SweetAlert.error();
                        } else {
                    ...
                    (err) => { console.error('Error deleting questionCategory', err); }
                );
            }
        },
        {
        ...
        },
        {
            ...
            handler: function (id, event) {
                this.callCustomFunction('_EDIT_WITH_MODAL_',
                    id,
                    `${__API_CFG__.LOCAL_URL}/dashboard/quizzes/question/category/get`,
                    (res) => {
                        console.log('res: ', res);
                    },
                    (err) => { console.error('Error editing questionCategory', err); },
                );
            }
        }
    ],
...
function createQuestionCategory() {
    FunctionUtility.closeModalWithButton('create-modal', '.close-modal', () => {
        FunctionUtility.clearForm('#create-questionCategory-form');
    });

    const createQuestionCategoryConfig = {
        formSelector: '#create-questionCategory-form',
        externalButtonSelector: '#create-questionCategory-button',
        endpoint: `${__API_CFG__.LOCAL_URL}/dashboard/quizzes/question/category/store`,
        feedback: true,
        onSuccess: (res) => {
            ...
            questionCategoryDataTable.reload();
        },
        onError: (err) => { console.error('Error adding questionCategory', err); },
    };

    const form = new $SingleFormPostController(createQuestionCategoryConfig);
    form.init();
}
...
const editQuestionCategory = () => {
    ...
    const editQuestionCategoryConfig = {
        formSelector: '#edit-questionCategory-form',
        externalButtonSelector: '#edit-questionCategory-button',
        endpoint: `${__API_CFG__.LOCAL_URL}/dashboard/quizzes/question/category/edit`,
        feedback: true,
        onSuccess: (res) => {
            Toast.showNotificationToast('', res.message)
            FunctionUtility.closeModal('edit-modal', () => {
                FunctionUtility.clearForm('#edit-questionCategory-form');
            });
            questionCategoryDataTable.reload();
        },
        onError: (err) => { console.error('Error editing questionCategory', err); },
    };

    const form = new $SingleFormPostController(editQuestionCategoryConfig);
    form.init();
}
