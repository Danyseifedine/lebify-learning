...
<!---------------------------
    Title
---------------------------->
@section('title', 'CourseRelatedChannel')
...
@section('toolbar')
    @include('dashboard.common.toolbar', [
        'title' => 'CourseRelatedChannel',
        'currentPage' => 'CourseRelatedChannel Management',
    ])
@endsection
...
@php
    $columns = ['course_id', 'channel_name', 'url', 'actions'];
@endphp
...
---------------------------->
@section('content')
    <x-lebify-table id="courserelatedchannelTable" :columns="$columns" {{-- create="true"                         // BY DEFAULT TRUE
...
---------------------------->
@push('scripts')
<script src="{{ asset('js/dashboard/course/relatedChannel.js') }}" type="module" defer></script>
@endpush
