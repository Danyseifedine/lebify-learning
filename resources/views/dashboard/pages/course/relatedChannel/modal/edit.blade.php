<form id="edit-courseRelatedChannel-form" form-id="editForm" http-request
    route="{{ route('dashboard.courses.relatedChannels.update') }}" identifier="single-form-post-handler" feedback
    close-modal success-toast on-success="RDT">
    <input type="hidden" name="id" id="id" value="{{ $courseRelatedChannel->id }}">
    ...
    <div class="mb-3">
        ...
        <select class="form-control form-control-solid" name="course_id" id="course_id" data-control="select2"
            ...
            @foreach ($courses as $course)
                <option value="{{ $course->id }}" {{ $courseRelatedChannel->course_id == $course->id ? 'selected' : '' }}>
                    {{ $course->title }}</option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control form-control-solid" placeholder="Enter Channel Name"
            ...
            value="{{ $courseRelatedChannel->channel_name }}">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control form-control-solid" placeholder="Enter Url" name="url"
            id="url" feedback-id="url-feedback" value="{{ $courseRelatedChannel->url }}">
